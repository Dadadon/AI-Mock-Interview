  ...
  return (
    <main className="flex flex-col gap-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {jobs?.map((job: any) => (
          <div key={job.id} className="card dark-gradient p-5 rounded-2xl border border-input">
            <h2 className="text-xl font-bold text-primary-200">{job.title}</h2>
            <p className="text-light-400 mt-2">{job.location}</p>
            <button className="blue-gradient-dark py-2 px-4 rounded-lg mt-4 w-full">Apply Now</button>
          </div>
        ))}
